...
class ensemble(object):
    ...
    def __init__(self, gnnsubnet=None, method="graphcheb", epoch_nr: int = 50, niter: int = 1, verbose: int = 0) -> None:
        ...
        for xx in range(len(self.modules)):
            ...
            # print(e1_final)
            graphs  = []
            edge_index = torch.tensor(np.array([e0_final, e1_final]), dtype=torch.long).to(device)
            ...
            for yy in range(len(gnnsubnet.dataset)):
                ...
                graphs.append(Data(x=torch.tensor(x_sub, dtype=torch.float).to(device), #x_sub.to(device),
                edge_index=edge_index, y=data_c.y.to(device)))
    ...
    def train(self, epoch_nr=40):

        train_data = self.train_data
        acc = list()

        for xx in range(len(self.ensemble)):
            self.ensemble[xx].train(method=self.ensemble[xx].classifier, epoch_nr=epoch_nr)
    ...
    def predict(self, gnnsubnet_test):
        ...
        for xx in range(len(self.ensemble)):
            ...
            testgraphs=[]
            edge_index_ens = self.ensemble[xx].dataset[0].edge_index.to(device)
            for yy in range(len(copy_test.dataset)):
                testgraphs.append(Data(x=torch.tensor(np.array(copy_test.dataset[yy].x.detach().cpu())[self.ensemble[xx].modules], dtype = torch.float).to(device),
                                       edge_index=edge_index_ens, #torch.tensor(np.array(self.ensemble[xx].dataset[0].edge_index.detach().cpu()), dtype=torch.long),
                                       y=copy_test.dataset[yy].y.to(device)))
